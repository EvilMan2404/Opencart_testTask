...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'additional_name'  => $query->row['additional_name'],
				'sku'              => $query->row['sku'],
				'upc'              => $query->row['upc'],
				'ean'              => $query->row['ean'],
				'jan'              => $query->row['jan'],
    ...
    /**
     * Получаем автоматически сгенерированный список рекомендуемых товаров
     * @param int $product_id
     * @return array
     */
    public function getAutoRelatedProducts(int $product_id) {
        $product_data = array();
        // Получили айди нужной нам категории (главной)
        $main_category_query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . $product_id . "' AND main_category = 1");
        $main_category_id = $main_category_query->row['category_id'];
        // Получили все товары из этой категории.
        // TODO: лимит можно закинуть в конфиги в админку, при необходимости его менять
        $query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product_to_category WHERE category_id = '" . (int)$main_category_id . "' ORDER BY RAND() LIMIT 10");
        foreach ($query->rows as $result) {
            $product_data[$result['product_id']] = $this->getProduct($result['product_id']);
        }
